...
Приложение автоматически ищет строку заголовков в первых 30 строках файла, поэтому Excel может содержать служебные строки над таблицей. Названия заголовков нормализуются: допускаются лишние пробелы, переносы строк и частые опечатки вроде `Артиккул`. В Excel должны быть колонки:
